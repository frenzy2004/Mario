...
const polishChecks: PolishRegressionCheck[] = [
  ...
  {
    name: "keeps level-27 encounter pacing varied",
    run: (levels) => {
      const level = getLevel(levels, "level-27");
      const summary = getLevelContentSummary(level);
      expect(summary.enemyCount).toBeGreaterThanOrEqual(12);
  ...
  {
    name: "keeps level-28 encounter pacing varied",
    run: (levels) => {
      const level = getLevel(levels, "level-28");
      const summary = getLevelContentSummary(level);
      expect(summary.enemyCount).toBeGreaterThanOrEqual(12);
  ...
  {
    name: "keeps level-29 encounter pacing varied",
    run: (levels) => {
      const level = getLevel(levels, "level-29");
      const summary = getLevelContentSummary(level);
      expect(summary.enemyCount).toBeGreaterThanOrEqual(12);
  ...
  {
    name: "keeps level-30 encounter pacing varied",
    run: (levels) => {
      const level = getLevel(levels, "level-30");
      const summary = getLevelContentSummary(level);
      expect(summary.enemyCount).toBeGreaterThanOrEqual(12);
  ...
  {
    name: "keeps level-31 encounter pacing varied",
    run: (levels) => {
      const level = getLevel(levels, "level-31");
      const summary = getLevelContentSummary(level);
      expect(summary.enemyCount).toBeGreaterThanOrEqual(12);
  ...
  {
    name: "keeps level-32 encounter pacing varied",
    run: (levels) => {
      const level = getLevel(levels, "level-32");
      const summary = getLevelContentSummary(level);
      expect(summary.enemyCount).toBeGreaterThanOrEqual(12);
  ...
  {
    name: "keeps level-33 encounter pacing varied",
    run: (levels) => {
      const level = getLevel(levels, "level-33");
      const summary = getLevelContentSummary(level);
      expect(summary.enemyCount).toBeGreaterThanOrEqual(12);
  ...
  {
    name: "keeps level-34 encounter pacing varied",
    run: (levels) => {
      const level = getLevel(levels, "level-34");
      const summary = getLevelContentSummary(level);
      expect(summary.enemyCount).toBeGreaterThanOrEqual(12);
  ...
  {
    name: "keeps level-35 encounter pacing varied",
    run: (levels) => {
      const level = getLevel(levels, "level-35");
      const summary = getLevelContentSummary(level);
      expect(summary.enemyCount).toBeGreaterThanOrEqual(12);
  ...
  {
    name: "keeps level-36 encounter pacing varied",
    run: (levels) => {
      const level = getLevel(levels, "level-36");
      const summary = getLevelContentSummary(level);
      expect(summary.enemyCount).toBeGreaterThanOrEqual(12);
  ...
  {
    name: "keeps level-37 encounter pacing varied",
    run: (levels) => {
      const level = getLevel(levels, "level-37");
      const summary = getLevelContentSummary(level);
      expect(summary.enemyCount).toBeGreaterThanOrEqual(12);
  ...
  {
    name: "keeps level-38 encounter pacing varied",
    run: (levels) => {
      const level = getLevel(levels, "level-38");
      const summary = getLevelContentSummary(level);
      expect(summary.enemyCount).toBeGreaterThanOrEqual(12);
  ...
  {
    name: "keeps level-40 encounter pacing varied",
    run: (levels) => {
      const level = getLevel(levels, "level-40");
      const summary = getLevelContentSummary(level);
      expect(summary.enemyCount).toBeGreaterThanOrEqual(12);
  ...
  {
    name: "keeps level-41 encounter pacing varied",
    run: (levels) => {
      const level = getLevel(levels, "level-41");
      const summary = getLevelContentSummary(level);
      expect(summary.enemyCount).toBeGreaterThanOrEqual(12);
  ...
  {
    name: "keeps level-42 encounter pacing varied",
    run: (levels) => {
      const level = getLevel(levels, "level-42");
      const summary = getLevelContentSummary(level);
      expect(summary.enemyCount).toBeGreaterThanOrEqual(12);
